...
static int cmp_integer_list(struct betree_integer_list* l1, struct betree_integer_list* l2)
{
    ...
    for (std::size_t i = 0; i < l1->count; i++) {
...
static int special_expr_cmp(struct ast_special_expr* s1, struct ast_special_expr* s2)
{
    ...
    switch (s1->type) {
        ...
        case AST_SPECIAL_STRING:
            ...
            return std::strcmp(s1->string.pattern, s2->string.pattern);
        default:
            std::abort();
    }
}
...
{
    auto* n1 = static_cast<struct ast_node*>(const_cast<void*>(p1));
    auto* n2 = static_cast<struct ast_node*>(const_cast<void*>(p2));
    ...
    switch (n1->type) {
        case AST_TYPE_COMPARE_EXPR:
            return compare_expr_cmp(&n1->compare_expr, &n2->compare_expr);
        case AST_TYPE_EQUALITY_EXPR:
            return equality_expr_cmp(&n1->equality_expr, &n2->equality_expr);
        case AST_TYPE_BOOL_EXPR:
            return bool_expr_cmp(&n1->bool_expr, &n2->bool_expr);
        case AST_TYPE_SET_EXPR:
            return set_expr_cmp(&n1->set_expr, &n2->set_expr);
        case AST_TYPE_LIST_EXPR:
            return list_expr_cmp(&n1->list_expr, &n2->list_expr);
        case AST_TYPE_SPECIAL_EXPR:
            return special_expr_cmp(&n1->special_expr, &n2->special_expr);
        case AST_TYPE_IS_NULL_EXPR:
            return is_null_expr_cmp(&n1->is_null_expr, &n2->is_null_expr);
        default:
            std::abort();
    }
}
