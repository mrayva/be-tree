...
/*
 * The ids filter is expected to be sorted in ascending order and treated as a set.
 * Unsorted or duplicate-heavy filters are not a guaranteed contract.
 */
...
// see betree_refresh_undefined() in tree.h) is recomputed in place instead
// of being allocated and freed fresh on every call - meant for a caller
// doing many searches against the same tree back to back (e.g. one per row
// of a batch). Does NOT reset `report` itself - call betree_reset_report()
// first, exactly as make_report() is called once for the non-reusing path.
bool betree_search_with_event_reusing(const struct betree* betree,
    struct betree_event* event, struct report* report, uint64_t* undefined_scratch);
