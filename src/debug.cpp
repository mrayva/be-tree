...
// Use C linkage for public functions
extern "C" {
...
{
    // C++ conversion: Use std::string for RAII memory management
    size_t len = strlen(input);
    std::string escaped_str;
    escaped_str.reserve(len + 1);
    ...
    // Allocate and return C-string for compatibility with existing code
    // The caller is responsible for freeing this with bfree()
    char* result = static_cast<char*>(bmalloc((len + 1) * sizeof(char)));
    if(result == nullptr) {
        fprintf(stderr, "%s bmalloc failed", __func__);
        abort();
    }
    strcpy(result, escaped_str.c_str());
    return result;
}
