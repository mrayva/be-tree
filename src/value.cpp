#include <cinttypes>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cstring>

#include "alloc.h"
#include "ast.h"
#include "betree.h"
#include "utils.h"
#include "value.h"  // This will include value.hpp when compiling as C++

extern "C" {

struct betree_integer_list* make_integer_list()
{
    auto* value = static_cast<struct betree_integer_list*>(bcalloc(sizeof(struct betree_integer_list)));
    if (value == nullptr) {
        std::fprintf(stderr, "%s bcalloc failed", __func__);
        std::abort();
    }
    return value;
}

struct betree_string_list* make_string_list()
{
    auto* value = static_cast<struct betree_string_list*>(bcalloc(sizeof(struct betree_string_list)));
    if (value == nullptr) {
        std::fprintf(stderr, "%s bcalloc failed", __func__);
        std::abort();
    }
    return value;
}

struct betree_segments* make_segments()
{
    auto* value = static_cast<struct betree_segments*>(bcalloc(sizeof(struct betree_segments)));
    if (value == nullptr) {
        std::fprintf(stderr, "%s bcalloc failed", __func__);
        std::abort();
    }
    return value;
}

struct betree_frequency_caps* make_frequency_caps()
{
    auto* value = static_cast<struct betree_frequency_caps*>(bcalloc(sizeof(struct betree_frequency_caps)));
    if (value == nullptr) {
        std::fprintf(stderr, "%s bcalloc failed", __func__);
        std::abort();
    }
    return value;
}

void add_integer_list_value(std::int64_t integer, struct betree_integer_list* list)
{
    if (list->count == 0) {
        list->integers = static_cast<std::int64_t*>(bcalloc(sizeof(std::int64_t)));
        if (list->integers == nullptr) {
            std::fprintf(stderr, "%s bcalloc failed", __func__);
            std::abort();
        }
    }
    else {
        auto* integers = static_cast<std::int64_t*>(brealloc(list->integers, sizeof(std::int64_t) * (list->count + 1)));
        if (integers == nullptr) {
            std::fprintf(stderr, "%s brealloc failed", __func__);
            std::abort();
        }
        list->integers = integers;
    }
    list->integers[list->count] = integer;
    list->count++;
}

char* integer_list_value_to_string(struct betree_integer_list* list)
{
    char* string = nullptr;
    for (std::size_t i = 0; i < list->count; i++) {
        char* new_string;
        if (i != 0) {
            if (basprintf(&new_string, "%s, %ld", string, list->integers[i]) < 0) {
                std::abort();
            }
            bfree(string);
        }
        else {
            if (basprintf(&new_string, "%ld", list->integers[i]) < 0) {
                std::abort();
            }
        }
        string = new_string;
    }
    return string;
}

void add_string_list_value(struct string_value string, struct betree_string_list* list)
{
    if (list->count == 0) {
        list->strings = static_cast<struct string_value*>(bcalloc(sizeof(struct string_value)));
        if (list->strings == nullptr) {
            std::fprintf(stderr, "%s bcalloc failed", __func__);
            std::abort();
        }
    }
    else {
        auto* strings
            = static_cast<struct string_value*>(brealloc(list->strings, sizeof(struct string_value) * (list->count + 1)));
        if (strings == nullptr) {
            std::fprintf(stderr, "%s brealloc failed", __func__);
            std::abort();
        }
        list->strings = strings;
    }
    list->strings[list->count] = string;
    list->count++;
}

char* string_list_value_to_string(struct betree_string_list* list)
{
    char* string = nullptr;
    for (std::size_t i = 0; i < list->count; i++) {
        char* new_string;
        if (i != 0) {
            if (basprintf(&new_string, "%s, \"%s\"", string, list->strings[i].string) < 0) {
                std::abort();
            }
            bfree(string);
        }
        else {
            if (basprintf(&new_string, "\"%s\"", list->strings[i].string) < 0) {
                std::abort();
            }
        }
        string = new_string;
    }
    return string;
}

void add_segment(struct betree_segment* segment, struct betree_segments* list)
{
    if (list->size == 0) {
        list->content = static_cast<struct betree_segment**>(bcalloc(sizeof(struct betree_segment*)));
        if (list->content == nullptr) {
            std::fprintf(stderr, "%s bcalloc failed", __func__);
            std::abort();
        }
    }
    else {
        auto* content = static_cast<struct betree_segment**>(brealloc(list->content, sizeof(struct betree_segment*) * (list->size + 1)));
        if (content == nullptr) {
            std::fprintf(stderr, "%s brealloc failed", __func__);
            std::abort();
        }
        list->content = content;
    }
    list->content[list->size] = segment;
    list->size++;
}

void add_frequency(struct betree_frequency_cap* frequency, struct betree_frequency_caps* list)
{
    if (list->size == 0) {
        list->content = static_cast<struct betree_frequency_cap**>(bcalloc(sizeof(struct betree_frequency_cap*)));
        if (list->content == nullptr) {
            std::fprintf(stderr, "%s bcalloc failed", __func__);
            std::abort();
        }
    }
    else {
        auto* content
            = static_cast<struct betree_frequency_cap**>(brealloc(list->content, sizeof(struct betree_frequency_cap*) * (list->size + 1)));
        if (content == nullptr) {
            std::fprintf(stderr, "%s brealloc failed", __func__);
            std::abort();
        }
        list->content = content;
    }
    list->content[list->size] = frequency;
    list->size++;
}

struct betree_segment* make_segment(std::int64_t id, std::int64_t timestamp)
{
    auto* segment = static_cast<struct betree_segment*>(bmalloc(sizeof(struct betree_segment)));
    segment->id = id;
    segment->timestamp = timestamp;
    return segment;
}

struct betree_frequency_cap* make_frequency_cap(const char* stype,
    std::uint32_t id,
    struct string_value ns,
    bool timestamp_defined,
    std::int64_t timestamp,
    std::uint32_t value)
{
    enum frequency_type_e type = get_type_from_string(stype);
    return make_frequency_cap_with_type(type, id, ns, timestamp_defined, timestamp, value);
}

struct betree_frequency_cap* make_frequency_cap_with_type(enum frequency_type_e type,
    std::uint32_t id,
    struct string_value ns,
    bool timestamp_defined,
    std::int64_t timestamp,
    std::uint32_t value)
{
    auto* frequency_cap = static_cast<struct betree_frequency_cap*>(bmalloc(sizeof(struct betree_frequency_cap)));
    frequency_cap->type = type;
    frequency_cap->id = id;
    frequency_cap->ns = ns;
    frequency_cap->timestamp_defined = timestamp_defined;
    frequency_cap->timestamp = timestamp;
    frequency_cap->value = value;
    return frequency_cap;
}

enum frequency_type_e get_type_from_string(const char* stype)
{
    if (std::strcmp(stype, "advertiser") == 0) {
        return FREQUENCY_TYPE_ADVERTISER;
    }
    if (std::strcmp(stype, "advertiser:ip") == 0) {
        return FREQUENCY_TYPE_ADVERTISERIP;
    }
    if (std::strcmp(stype, "campaign") == 0) {
        return FREQUENCY_TYPE_CAMPAIGN;
    }
    if (std::strcmp(stype, "campaign:ip") == 0) {
        return FREQUENCY_TYPE_CAMPAIGNIP;
    }
    if (std::strcmp(stype, "campaign_group") == 0) {
        return FREQUENCY_TYPE_CAMPAIGN_GROUP;
    }
    if (std::strcmp(stype, "campaign_group:ip") == 0) {
        return FREQUENCY_TYPE_CAMPAIGN_GROUPIP;
    }
    if (std::strcmp(stype, "product") == 0) {
        return FREQUENCY_TYPE_PRODUCT;
    }
    if (std::strcmp(stype, "product:ip") == 0) {
        return FREQUENCY_TYPE_PRODUCTIP;
    }
    if (std::strcmp(stype, "flight") == 0) {
        return FREQUENCY_TYPE_FLIGHT;
    }
    if (std::strcmp(stype, "flight:ip") == 0) {
        return FREQUENCY_TYPE_FLIGHTIP;
    }
    return FREQUENCY_TYPE_INVALID;
}

void free_integer_list(struct betree_integer_list* value)
{
    bfree(value->integers);
    bfree(value);
}

void free_string_list(struct betree_string_list* value)
{
    for (std::size_t i = 0; i < value->count; i++) {
        bfree(const_cast<char*>(value->strings[i].string));
    }
    bfree(value->strings);
    bfree(value);
}

void free_segment(struct betree_segment* value)
{
    bfree(value);
}

void free_segments(struct betree_segments* value)
{
    for (std::size_t i = 0; i < value->size; i++) {
        if (value->content[i] != nullptr) {
            bfree(value->content[i]);
        }
    }
    bfree(value->content);
    bfree(value);
}

void free_frequency_cap(struct betree_frequency_cap* value)
{
    bfree(const_cast<char*>(value->ns.string));
    bfree(value);
}

void free_frequency_caps(struct betree_frequency_caps* value)
{
    for (std::size_t i = 0; i < value->size; i++) {
        if (value->content[i] != nullptr) {
            free_frequency_cap(value->content[i]);
        }
    }
    bfree(value->content);
    bfree(value);
}

void free_value(struct value value)
{
    switch (value.value_type) {
        case BETREE_INTEGER_LIST: {
            free_integer_list(value.integer_list_value);
            break;
        }
        case BETREE_STRING_LIST: {
            free_string_list(value.string_list_value);
            break;
        }
        case BETREE_STRING: {
            bfree(const_cast<char*>(value.string_value.string));
        }
        case BETREE_BOOLEAN:
        case BETREE_INTEGER:
        case BETREE_INTEGER_ENUM:
        case BETREE_FLOAT: {
            break;
        }
        case BETREE_SEGMENTS: {
            free_segments(value.segments_value);
            break;
        }
        case BETREE_FREQUENCY_CAPS: {
            free_frequency_caps(value.frequency_caps_value);
            break;
        }
        default:
            std::abort();
    }
}

char* segment_value_to_string(struct betree_segment* segment)
{
    char* string = nullptr;
    if (basprintf(&string, "[%ld, %ld]", segment->id, segment->timestamp) < 0) {
        std::abort();
    }
    return string;
}

char* segments_value_to_string(struct betree_segments* list)
{
    char* string = nullptr;
    for (std::size_t i = 0; i < list->size; i++) {
        char* new_string;
        char* segment = segment_value_to_string(list->content[i]);
        if (i != 0) {
            if (basprintf(&new_string, "%s, %s", string, segment) < 0) {
                std::abort();
            }
            bfree(string);
        }
        else {
            if (basprintf(&new_string, "%s", segment) < 0) {
                std::abort();
            }
        }
        bfree(segment);
        string = new_string;
    }
    return string;
}

char* frequency_cap_to_string(struct betree_frequency_cap* cap)
{
    char* string = nullptr;
    const char* type = frequency_type_to_string(cap->type);
    if (cap->timestamp_defined) {
        if (basprintf(&string, "[[\"%s\", %u, \"%s\"], %u, %ld]", type, cap->id, cap->ns.string, cap->value, cap->timestamp) < 0) {
            std::abort();
        }
    }
    else {
        if (basprintf(&string, "[[\"%s\", %u, \"%s\"], %u, undefined]", type, cap->id, cap->ns.string, cap->value) < 0) {
            std::abort();
        }
    }
    return string;
}

char* frequency_caps_value_to_string(struct betree_frequency_caps* list)
{
    char* string = nullptr;
    for (std::size_t i = 0; i < list->size; i++) {
        char* new_string;
        char* cap = frequency_cap_to_string(list->content[i]);
        if (i != 0) {
            if (basprintf(&new_string, "%s, %s", string, cap) < 0) {
                std::abort();
            }
            bfree(string);
        }
        else {
            if (basprintf(&new_string, "%s", cap) < 0) {
                std::abort();
            }
        }
        bfree(cap);
        string = new_string;
    }
    return string;
}

void remove_duplicates_integer_list(struct betree_integer_list* list)
{
    if (list->count == 0) {
        return;
    }
    std::size_t r = 0;
    for (std::size_t i = 1; i < list->count; i++) {
        if (list->integers[r] != list->integers[i]) {
            list->integers[++r] = list->integers[i];
        }
    }
    list->count = r + 1;
}

void sort_integer_list(struct betree_integer_list* list)
{
    if(list->count < 2) {
        return;
    }
    qsort(list->integers, list->count, sizeof(std::int64_t), icmpfunc);
}

void sort_and_remove_duplicate_integer_list(struct betree_integer_list* list)
{
    sort_integer_list(list);
    remove_duplicates_integer_list(list);
}

void remove_duplicates_string_list(struct betree_string_list* list)
{
    if (list->count == 0) {
        return;
    }
    std::size_t r = 0;
    for (std::size_t i = 1; i < list->count; i++) {
        if (list->strings[r].str != list->strings[i].str) {
            list->strings[++r] = list->strings[i];
        }
        else {
            bfree(const_cast<char*>(list->strings[i].string));
        }
    }
    list->count = r + 1;
}

void sort_string_list(struct betree_string_list* list)
{
    if(list->count < 2) {
        return;
    }
    qsort(list->strings, list->count, sizeof(struct string_value), scmpfunc);
}

void sort_and_remove_duplicate_string_list(struct betree_string_list* list)
{
    sort_string_list(list);
    remove_duplicates_string_list(list);
}

} // extern "C"
